// add a dropdown in the summary tab to select the month to show summary for or all the months
...
const ExpenseTracker = () => {
  ...
  const [currentMonth, setCurrentMonth] = useState(() => localStorage.getItem('currentMonth') || new Date().toISOString().slice(0, 7));
...
// Filter transactions based on search query
  ...
  if (currentMonth && currentMonth !== 'all') {
    filteredTransactions = transactions.filter(t => t.date.startsWith(currentMonth));
  }
...
const getCategoryBreakdown = () => {
  ...
  filteredTransactions.forEach(t => {
    if (t.category === 'Carry Over') return; // Skip Carry Over category
...
// Add helper functions for dropdown
const getUniqueMonths = () => {
  const months = new Set();
  transactions.forEach(transaction => {
    months.add(transaction.date.slice(0, 7)); // Extract YYYY-MM
  });
  return [...months].sort((a, b) => b.localeCompare(a)); // Sort newest to oldest
};
...
// Update localStorage for currentMonth
useEffect(() => {
            ...
            <div className="space-y-6">
              {categoryBreakdown.incomeCategories.length > 0 && (
                <div className="bg-white border border-gray-200 rounded-lg p-6">
                  <h3 className="text-lg font-semibold mb-4 text-green-600">Top Income Categories {currentMonth === 'all' ? 'Across All Months' : 'This Month'}</h3>
                  <div className="space-y-3">
                    {categoryBreakdown.incomeCategories.map((item) => {
                      const categoryTransactions = transactions
                        .filter(t => t.category === item.category && t.type === 'income')
                        .filter(t => currentMonth === 'all' || t.date.startsWith(currentMonth));
                      return (
                          ...
                          <div className="flex items-center justify-between">
                            <div className="flex items-center space-x-3">
                              ...
                              <button
                                ...
                                className="flex items-center space-x-2 font-medium hover:text-green-700"
                              >
                                <span>{item.category}</span>
                                {collapsedCategories[item.category] ? (
                            ...
                            <div className="text-right">
                              ...
                              <div className="text-sm text-gray-600">
                                {monthlyData.income > 0 ? ((item.amount / monthlyData.income) * 100).toFixed(0) : 0}% of income
                              </div>
                            </div>
                          </div>
                          {!collapsedCategories[item.category] && (
              ...
              {categoryBreakdown.expenseCategories.length > 0 && (
                <div className="bg-white border border-gray-200 rounded-lg p-6">
                  <h3 className="text-lg font-semibold mb-4 text-red-600">Top Expense Categories {currentMonth === 'all' ? 'Across All Months' : 'This Month'}</h3>
                  <div className="space-y-3">
                        ...
                        <div key={item.category}>
                          <div className="flex items-center justify-between">
                            <div className="flex items-center space-x-3">
                              ...
                              <button
                                onClick={() => toggleCategoryCollapse(item.category)}
                                className="flex items-center space-x-2 font-medium hover:text-red-700"
                              >
                                <span>{item.category}</span>
                                {collapsedCategories[item.category] ? (
                                  <ChevronUp className="w-4 h-4" />
                            ...
                            <div className="text-right">
                              ...
                              <div className="text-sm text-gray-600">
                                {monthlyData.expenses > 0 ? ((item.amount / monthlyData.expenses) * 100).toFixed(0) : 0}% of expenses
                              </div>
                            </div>
                          </div>
                          {!collapsedCategories[item.category] && (
