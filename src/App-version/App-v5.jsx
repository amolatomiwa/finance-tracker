// I corrected the delete All Confirmation dialogue to appear in the setting tab and not in the transaction tab
// I added an on-key event to sumbit a transaction details when the enter button is clicked

import React, { useState, useEffect } from 'react';
import { Plus, Trash2, Settings, Calendar, TrendingUp, TrendingDown, PieChart, Search, Download, ChevronDown, ChevronUp, Eye, EyeOff, Edit2, AlertTriangle } from 'lucide-react';
  ...
  const addOrUpdateTransaction = () => {
    ...
    if (isEditing) {
      if (formData.type === 'transfer') {
        ...
        const updatedTransactions = transactions.filter(t => t.id !== editingTransactionId && t.id !== (relatedTransaction?.id));
        ...
        setTransactions([transferOut, transferIn, ...updatedTransactions]);
      } else {
        ...
        setTransactions(transactions.map(t => t.id === editingTransactionId ? updatedTransaction : t));
      }
    } else {
      if (formData.type === 'transfer') {
        if (!formData.toAccount || formData.account === formData.toAccount) return;
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <p className="font-medium">
                            {transaction.note || (transaction.category === 'Transfer' 
                              ? `Transfer ${transaction.type === 'income' ? 'from' : 'to'} ${transaction.fromAccount || transaction.toAccount}` 
                              : transaction.category)}
                          </p>
